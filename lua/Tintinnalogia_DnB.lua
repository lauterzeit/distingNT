-- Tintinnalogia Drum and Bass edition
--[[
...
return {
  ...
  init = function(self)
    ...
    return {
      ...
      inputNames  = { 'Clock', 'Changes', 'Rounds', 'Freeze' },
      ...
      parameters  = {
        ...
        {'Changes', {'Off', 'On'}, 2},
        {'Pause gate', {'Off', 'On'}, 1},
        {'Freeze mode', {'Momentary','Latch'}, 1},
        {'Freeze rate', 1, 8, 1},
        {'Freeze density', 0, 100, 100},
        {'Freeze jitter ms', 0, 10, 0},
      }
    }
  end,
  ...
  gate = function(self, input, rising)
    ...
    -- Input 4: Freeze
    if input == 4 then
      local mode = self.parameters[11]

      if mode == 1 then
        self.freeze_active = rising
        if rising then self.frozen_bell = self.last_bell end
      else
        if rising then
          self.freeze_latched = not self.freeze_latched
          self.freeze_active = self.freeze_latched
          if self.freeze_active then self.frozen_bell = self.last_bell end
        end
      end
      return
    end
    ...
    if input == 1 then
      if self.freeze_active and self.frozen_bell then
        local mult   = self.parameters[12]
        local prob   = self.parameters[13] / 100
        local jitter = self.parameters[14]

        for i = 1, mult do
          if math.random() < prob then
            if jitter > 0 then
              clock.sleep(((math.random()*2-1)*jitter)/1000)
            end
            self.trigger_time[self.frozen_bell] = 0.010
            self.trigger_velocity[self.frozen_bell] = 5.0
          end
        end
